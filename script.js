(function () {
  ...
  window.nextSong = function () {
    ...
    if (note) {
      ...
      note.style.transform = 'scale(0.75)';
      setTimeout(() => { note.style.transform = ''; note.style.animation = ''; }, 200);
    }
  ...
  function spawnConfetti() {
    ...
    for (let i = 0; i < 220; i++) {
      confettiPieces.push({
        x: cx + rand(-50, 50), y: cy,
        vx: rand(-9, 9), vy: rand(-18, -4),
        color: randItem(CONFETTI_COLORS),
        size: rand(5, 15), rot: rand(0, 360), rotV: rand(-12, 12),
        shape: Math.random() > 0.5 ? 'rect' : 'circle',
        alpha: 1, gravity: 0.45
      });
    }
    confettiActive = true;
  }
  ...
  function spawnBalloons() {
    ...
    setInterval(() => {
      ...
      balloons.push({
        x: rand(0, window.innerWidth), y: window.innerHeight + 60,
        vy: rand(-3, -1.2), vx: rand(-0.4, 0.4),
        r: rand(22, 42), color: randItem(BALLOON_COLORS),
        sway: rand(0, Math.PI * 2), swaySpeed: rand(0.01, 0.04), alpha: 1
      });
    }, 300);
  }
  ...
  function drawBalloon(b) {
    ...
    bCtx.beginPath(); bCtx.ellipse(b.x, b.y, b.r * 0.85, b.r, 0, 0, Math.PI * 2);
    bCtx.fillStyle = b.color; bCtx.fill();
    bCtx.beginPath(); bCtx.ellipse(b.x - b.r * 0.22, b.y - b.r * 0.3, b.r * 0.18, b.r * 0.25, -0.4, 0, Math.PI * 2);
